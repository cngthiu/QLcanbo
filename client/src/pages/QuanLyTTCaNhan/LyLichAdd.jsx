...
const LyLichAdd = () => {
  ...
  useEffect(() => {
    if (state) {
      setForm((prev) => ({
        ...
        ...state,
        MaDV: state.MaDV || "",
        NgaySinh: formatDateSafe(state.NgaySinh),
        NgayNhapNgu: formatDateSafe(state.NgayNhapNgu),
        NgayThamGiaCM: formatDateSafe(state.NgayThamGiaCM), // đúng key
        Ngaychinhthuc: formatDateSafe(state.Ngaychinhthuc),
        Ngayvaodoan: formatDateSafe(state.Ngayvaodoan),
        NgayCongTac: formatDateSafe(state.NgayCongTac),
        Ngayvaodang: formatDateSafe(state.Ngayvaodang),
      }));
    }
  }, [state]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      dateFields.forEach((field) => {
        if (!processedForm[field] || processedForm[field] === "") {
          processedForm[field] = null;
        } else {
          const date = new Date(processedForm[field]);
          processedForm[field] = isNaN(date.getTime())
            ? null
            : date.toISOString().slice(0, 10);
        }
      });

      await api.createCanBo(processedForm);
  ...
  return (
    <div className="p-6">
      <h1 className="text-xl font-bold text-blue-700 mb-6">TẠO LÝ LỊCH</h1>
      ...
      <div className="grid grid-cols-2 gap-6 text-sm">
        <div className="space-y-4">
          ...
          <div>
            ...
            >
              <option value="">-- Chọn --</option>
              <option value="Nam">Chỉ huy - TM</option>
              <option value="Nữ">Chính trị</option>
              <option value="Nữ">Hậu cần</option>
              <option value="Nữ">Kỹ thuật</option>
              <option value="Nữ">Giảng viên</option>
              <option value="Nữ">Nghiên cứu khoa học</option>
            </select>
          </div>
        </div>
